...
**Enregistre tes clics, scrolls, mouvements souris et frappes clavier — puis rejoue-les autant de fois que tu veux.**
...
![Plateforme : Windows 10/11](https://img.shields.io/badge/plateforme-Windows%2010%2F11-blue)
...
Récupère **[`ClickClick.exe` dans la dernière release](https://github.com/Shult/click_click/releases/latest)** et pose-le où tu veux : il crée `sessions/`, `settings.json` et `clickclick.log` à côté de lui. Windows 10/11, rien d'autre à installer. Les versions suivantes s'installeront depuis l'application.

> **L'exécutable n'est pas signé** et pose un hook clavier global : SmartScreen affichera un avertissement (« Informations complémentaires » → « Exécuter quand même ») et certains antivirus peuvent le signaler. Voir [Ce qu'il fait de tes frappes](#ce-quil-fait-de-tes-frappes) ci-dessous, ou [construire depuis les sources](#développement) si tu préfères ne pas me croire sur parole.

## Ce qu'il fait de tes frappes

Tout reste sur ta machine.

- Les sessions enregistrées sont de simples fichiers JSON posés à côté de l'exécutable. Tu peux les lire, les modifier, les supprimer.
- **Aucune télémétrie, aucune analytique, aucun compte.** La seule requête réseau que l'application émette est un `GET` anonyme vers l'API des releases GitHub, pour comparer son numéro de version à la dernière publiée. Avec `"update_check": false` dans `settings.json`, elle n'en émet aucune.
- Le hook clavier global est ce qui permet d'enregistrer les frappes et les raccourcis globaux. C'est aussi, inévitablement, ce qui fait ressembler le binaire à un keylogger pour un antivirus. Le code est ici, et `pyinstaller ClickClick.spec` reproduit l'exécutable.
...
Ou double-clique sur **`lancer.bat`** : il installe `uv` et les dépendances à la première ouverture, puis lance l'application.
...
Les contributions sont bienvenues — ouvre une issue d'abord pour tout ce qui est conséquent. Les commentaires du code sont en français ; les textes d'interface et les messages de journal sont en anglais.
...
Les releases sont construites par [`.github/workflows/release.yml`](.github/workflows/release.yml) au push d'un tag `v*`, et le SHA-256 de l'exécutable est publié dans les notes de version.
